...
lT = 157; %length of trace (lT register writes)
...
spF = spg-2; spL = spg+8; %look to verify guess in this range
load guess
for r = 1:8
    ham1 = hamming{r};
Gcorr = corr(ham1,(P(1:1e4,spF:spL)));
Gcorrs(r,:) = (sum(Gcorr,2)./10);
end
figure;plot(Gcorrs');title('2^8 guesses, one correct; only have range for correct sample point');xlabel('guess');ylabel('correlation');
