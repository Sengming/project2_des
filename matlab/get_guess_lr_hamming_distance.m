...
% from. This will be nT * 2 where the 2 columns are plaintext - ciphertext pairs.
  ...
  for i = 1:10000
    ...
    % At this point we have input_r. The next step is to generate the s-box guesses
    % sbox guesses are 64x4 in size.
    sbox_outputs = linspace(0, 63, 64);
    sbox_outputs = dec2bin(sbox_outputs(1,:));
    size(sbox_outputs)
